...
db_config = json.load(open('./../config.json', 'r'))
print(tuple(db_config.items()))
    ...
    for index, info in enumerate(table.findAll("tr")):
        ...
        if last_forecast_time:
            forecast_document["time"] = fetch_timestamp_from_date(
                last_forecast_time, info[0].text)
        else:
...
def fetch_active_cyclones():
    cyclone_data = []
    html_body = fetch_html_content(CYCLONE_INFO_URL, {"id": "content"}, "div", {
                                   "class": "basin_storms"})
...
def fetch_details_for_active_cyclones(data):
    for document in data:
        h3_elements = fetch_html_content(get_cyclone_detail_url(
            document["cyclone_id"]), {"class": "text_product_wrapper"}, "h3")
        ...
        if str(next_elemennt_for_forecast.text) != NO_DATA:
            ...
            document["forecast_data"] = convert_table_to_json(
                table, document["forecast_time"])

        if str(next_elemennt_for_track.text) != NO_DATA:
            document["track_data"] = convert_table_to_json(
